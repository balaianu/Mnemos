...
## CML, the operator map, and the reranker

CML is lossless for any *reader*, but its notation is a real problem for the *models* in the retrieval pipeline, and the problem has two tiers of severity.

For a multilingual embedder like e5-large the form is mildly out of distribution: it was trained on natural-language pairs (Wikipedia, MS MARCO, mC4) and saw prefixes like `F:`, `D:` and operators like `→` `∵` so rarely that they carry almost no learned meaning. For an English-vocabulary model (the BGE family, most small rerankers) it is worse than out of distribution: symbols like `∵ ∴ △ ⚠ ✓ ✗` are not in the vocabulary at all, so every one of them collapses to the same shared unknown token. A memory saying a migration was `✓` confirmed and one saying it was `✗` rejected become IDENTICAL to such a model at those positions. Measured on a real 720-memory store, that was 901 unknown tokens; the damage shows up as a precision failure, not a recall one, which is exactly the shape of the published ablation below.

**The operator map (default on since v10.30.0) removes most of this at the source, for every embedder.** Before embedding, the scoring copy of each memory has the operators spelled out as the words they stand for: `✓` becomes "confirmed", `∵` becomes "because". Stored content is never modified, what the agent reads back is unchanged, and the cost is about 0.1% extra tokens on the embedder side only. It is a no-op in the sense that e5 tokenizes the symbols fine, and measurably NOT a no-op in effect: words the model saw constantly beat symbols it technically recognizes, for e5 as much as for the English models. The reranker receives the same mapping automatically when a per-model probe finds its tokenizer cannot represent the operators; rerankers whose vocabularies carry the symbols natively score the raw text.

**The reranker is not optional if you care about R@1.** An earlier revision of this page called it "the cherry on top"; the 2026-08-19 benchmarks retired that framing. CML's structural prefixes are explicit relation markers a cross-encoder attends to, and the bi-encoder alone cannot replace that: measured on the stratified LongMemEval subset, R@1 is 81.29 without the reranker and 92.40 with it, while R@5 barely moves, which is why disabling it LOOKS cheap. R@5 is what gets retrieved; R@1 is what an agent actually reads first. The published ablation shows the same shape at its sharpest: with CML and no reranker, `single-session-preference` R@1 collapses to 53.33%, and adding the reranker takes it to 80.00%. You can still ship without a reranker (`MNEMOS_ENABLE_RERANK=0`) and land respectable R@5, but it is a real quality trade, not a garnish.

The combined picture:

- **CML alone, no reranker**: essentially the same R@5 as the no-CML lite mode, with **15% to 60% fewer tokens** stored and reinjected per memory (the spread depends on how narrative the source prose is), but the R@1 collapse above applies. Acceptable when context budget is everything and top-1 precision is not; know what you are trading.
...
CML is the default convention everything in Mnemos assumes, and in practice the system enforces it through machinery rather than through schema validation. The CLI hints in CML. The dedup pipeline matches CML subject prefixes for conflict detection. When the Nyx cycle merges a cluster of duplicates it unions their existing fact lines, so consolidated memories stay in the CML they were written in. Briefings, digests, and the topic map all render in CML. The MCP tool description teaches the agent CML conventions so it writes in CML directly.

So if you store a memory in plain prose, Mnemos will accept it and search it just fine, and it stays exactly as you wrote it; nothing rewrites a standalone memory in the background. What makes the corpus trend CML is that the agent is instructed to write CML in the first place, plus merges of duplicate clusters that union existing CML lines. **You can opt out**: set `MNEMOS_CML_MODE=off` and customize the agent's system prompt to write plain prose, and you have a prose-only deployment. But by default the corpus is CML because that is what gets written, and the rest of the system is built around that assumption.
...
- **Merges stay compact, but nothing rewrites a standalone memory.** When Phase 2 merges a cluster of duplicates, the default mechanical engine unions their existing fact lines (selection, not generation), so the merged super-memory is as compact as the CML lines that fed it; if you opt into the LLM merge engine (`MNEMOS_MERGE_ENGINE=llm`) its prompt emits CML too, and Phase 3/5 insights are written in CML. What the Nyx cycle does not do is convert a plain-prose memory that was never part of a merge: store one in free-form prose and it stays exactly as you wrote it. The store trends toward CML because the agent writes CML, not because a background pass rewrites the store
...
If you (or, more realistically, the AI agent on your behalf) write memories in CML, the system rewards you with smaller context bills, better dedup precision, and more reliable consolidation. If you do not, everything still works; you just leave the token savings on the table for those memories.
